...
export const profesor1 = async (dataSource: DataSource) => {
  try {
    ...
    const existingProfesor = await profesorRepository.findOne({
      where: { email: 'profesor1@example.com' },
    });

    if (existingProfesor) {
      console.log('Profesor 1 ya existe, omitiendo creación');
      return;
    }
    ...
    console.log('Profesor 1 creado exitosamente');
    ...
    console.error('Error creando profesor 1:', error);
  }
};
