  ...
  try {
    ...
    const cargasAcademicas = [
      ...
      },
      {
        carrera: carrera.nombre,
        asignatura: 'INGLÉS I',
        grupoId: gruposDisponibles[1]?.id || gruposDisponibles[0].id, // ✅ Asignar al primer grupo
      },
      {
        carrera: carrera.nombre,
        asignatura: 'ASIGNATURA INTEGRADORA',
        grupoId: gruposDisponibles[2]?.id || gruposDisponibles[0].id, // ✅ Asignar al segundo grupo o primero si no hay más
      },
    ];
